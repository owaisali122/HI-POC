/**
 * App Detail Ref / Reference Field – form schema helpers.
 * Exclude list: AppDetailRef.ts → APP_DETAIL_REF_EXCLUDE_TYPES
 */

import { APP_DETAIL_REF_EXCLUDE_TYPES } from '../components/formio/AppDetailRef'
...
export function getReferencableComponents(components: unknown[]): Array<{ key: string; label: string }> {
...
/** Cache for getFormSchemaForPreview by doc id to avoid repeated work on redraws. */
const previewSchemaCache = new Map<string, { display: string; components: unknown[] }>()
const PREVIEW_CACHE_MAX = 50

/**
 * Return form schema for Formio.createForm. Payload Forms use doc.schema (display + components).
 * Result is memoized by doc.id to avoid unnecessary work on repeated redraws.
 */
export function getFormSchemaForPreview(doc: Record<string, unknown> | null): { display: string; components: unknown[] } {
  if (!doc || typeof doc !== 'object') return { display: 'form', components: [] }
  const id = doc.id != null ? String(doc.id) : undefined
  if (id) {
    const cached = previewSchemaCache.get(id)
    if (cached) return cached
  }
  let result: { display: string; components: unknown[] }
  const schema = doc.schema as Record<string, unknown> | undefined
  if (schema && Array.isArray(schema.components)) {
    result = { display: String(schema.display || 'form'), components: schema.components as unknown[] }
  } else if (doc.display && Array.isArray(doc.components)) {
    result = { display: String(doc.display), components: doc.components as unknown[] }
  } else {
    const form = doc.form as Record<string, unknown> | undefined
    if (form && form.display && Array.isArray(form.components)) {
      result = { display: String(form.display), components: form.components as unknown[] }
    } else {
      const comps = getDocComponents(doc)
      result = { display: 'form', components: comps }
    }
  }
  if (id) {
    if (previewSchemaCache.size >= PREVIEW_CACHE_MAX) {
      const firstKey = previewSchemaCache.keys().next().value
      if (firstKey != null) previewSchemaCache.delete(firstKey)
    }
    previewSchemaCache.set(id, result)
  }
  return result
}

/** No-op (API compatibility). */
export async function runAppDetailRefInjection(schema: Record<string, unknown>): Promise<Record<string, unknown>> {
  return schema
}
